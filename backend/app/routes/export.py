...
class LaTeXExportRequest(BaseModel):
    """Request to export analysis results as LaTeX tables."""
    session_id: str
    results: List[AnalysisResult]
    summary: Dict[str, Any]
    table_caption: Optional[str] = None
    table_label: Optional[str] = None
    include_header: bool = True  # Include LaTeX preamble comments


@router.post("/export/latex/summary")
async def export_latex_summary_table(request: LaTeXExportRequest):
    """
    Export summary statistics as a LaTeX table.
    
    Returns a LaTeX tabular environment ready for inclusion in research papers.
    Compatible with standard LaTeX document classes.
    """
    try:
        results = request.results
        ...
        if request.include_header:
            latex_lines.append("% Summary Statistics Table")
            latex_lines.append("% Generated by GeoHIS Research Platform")
            latex_lines.append(f"% Date: {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}")
            latex_lines.append("")
        
        latex_lines.append("\\begin{table}[htbp]")
        latex_lines.append("\\centering")
        
        caption = request.table_caption or "Summary Statistics of Susceptibility Analysis"
        latex_lines.append(f"\\caption{{{escape_latex(caption)}}}")
        
        label = request.table_label or f"tab:summary_{request.session_id[:8]}"
        latex_lines.append(f"\\label{{{label}}}")
        
        latex_lines.append("\\begin{tabular}{lrr}")
        latex_lines.append("\\hline")
        latex_lines.append("\\textbf{Statistic} & \\textbf{Flood (\\%)} & \\textbf{Landslide (\\%)} \\\\")
        latex_lines.append("\\hline")
        
        # Add statistics rows
        stats = [
            ("Count", len(flood_values), len(landslide_values)),
            ("Mean", round(np.mean(flood_values), 2), round(np.mean(landslide_values), 2)),
            ("Std. Dev.", round(np.std(flood_values), 2), round(np.std(landslide_values), 2)),
            ("Min", round(np.min(flood_values), 2), round(np.min(landslide_values), 2)),
            ("25th Percentile", round(np.percentile(flood_values, 25), 2), round(np.percentile(landslide_values, 25), 2)),
            ("Median", round(np.median(flood_values), 2), round(np.median(landslide_values), 2)),
            ("75th Percentile", round(np.percentile(flood_values, 75), 2), round(np.percentile(landslide_values, 75), 2)),
            ("Max", round(np.max(flood_values), 2), round(np.max(landslide_values), 2)),
        ]
        
        for stat_name, flood_val, landslide_val in stats:
            latex_lines.append(f"{stat_name} & {flood_val} & {landslide_val} \\\\")
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=summary_table_{request.session_id[:8]}.tex"
            }
        )
...
@router.post("/export/latex/risk-classification")
async def export_latex_risk_classification_table(request: LaTeXExportRequest):
    """
    Export risk classification frequency table as LaTeX.
    
    Shows distribution of locations across risk categories for flood,
    landslide, and combined hazards.
    """
    try:
        results = request.results
        ...
        if request.include_header:
            latex_lines.append("% Risk Classification Table")
            latex_lines.append("% Generated by GeoHIS Research Platform")
            latex_lines.append(f"% Date: {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}")
            latex_lines.append("")
        ...
        caption = request.table_caption or "Distribution of Locations by Risk Classification"
        latex_lines.append(f"\\caption{{{escape_latex(caption)}}}")
        
        label = request.table_label or f"tab:risk_class_{request.session_id[:8]}"
        latex_lines.append(f"\\label{{{label}}}")
        
        latex_lines.append("\\begin{tabular}{lrrrrrr}")
        latex_lines.append("\\hline")
        latex_lines.append("\\textbf{Risk Class} & \\multicolumn{2}{c}{\\textbf{Flood}} & \\multicolumn{2}{c}{\\textbf{Landslide}} & \\multicolumn{2}{c}{\\textbf{Combined}} \\\\")
        latex_lines.append(" & n & \\% & n & \\% & n & \\% \\\\")
        latex_lines.append("\\hline")
        
        for risk in risk_order:
            flood_n = flood_classes.get(risk, 0)
            landslide_n = landslide_classes.get(risk, 0)
            combined_n = combined_classes.get(risk, 0)
            
            if flood_n > 0 or landslide_n > 0 or combined_n > 0:
                flood_pct = round(flood_n / total * 100, 1) if total > 0 else 0
                landslide_pct = round(landslide_n / total * 100, 1) if total > 0 else 0
                combined_pct = round(combined_n / total * 100, 1) if total > 0 else 0
                
                latex_lines.append(f"{escape_latex(risk)} & {flood_n} & {flood_pct} & {landslide_n} & {landslide_pct} & {combined_n} & {combined_pct} \\\\")
        
        latex_lines.append("\\hline")
        latex_lines.append(f"\\textbf{{Total}} & {total} & 100.0 & {total} & 100.0 & {total} & 100.0 \\\\")
        latex_lines.append("\\hline")
        latex_lines.append("\\end{tabular}")
        latex_lines.append("\\end{table}")
        ...
        return Response(
            ...
            media_type="text/plain",
            headers={
                "Content-Disposition": f"attachment; filename=risk_classification_{request.session_id[:8]}.tex"
            }
        )
...
@router.post("/export/latex/full-results")
async def export_latex_full_results_table(request: LaTeXExportRequest):
    """
    Export full analysis results as a LaTeX longtable.
    
    Uses the longtable package for multi-page tables.
    Includes all locations with their susceptibility values.
    
    Note: Requires \\usepackage{longtable} in your LaTeX preamble.
    """
    try:
        results = request.results
        ...
        if request.include_header:
            latex_lines.append("% Full Results Table (requires longtable package)")
            latex_lines.append("% Add \\usepackage{longtable} to your preamble")
            latex_lines.append("% Generated by GeoHIS Research Platform")
            latex_lines.append(f"% Date: {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}")
            latex_lines.append(f"% Total locations: {len(results)}")
            latex_lines.append("")
        
        caption = request.table_caption or "Complete Analysis Results for Study Area"
        label = request.table_label or f"tab:full_results_{request.session_id[:8]}"
        
        latex_lines.append("\\begin{longtable}{lrrrrrr}")
        latex_lines.append(f"\\caption{{{escape_latex(caption)}}} \\label{{{label}}} \\\\")
        latex_lines.append("\\hline")
        latex_lines.append("\\textbf{Location} & \\textbf{Lat} & \\textbf{Lon} & \\textbf{Flood} & \\textbf{F.Class} & \\textbf{Landslide} & \\textbf{L.Class} \\\\")
        latex_lines.append("\\hline")
        latex_lines.append("\\endfirsthead")
        
        # Header for continuation pages
        latex_lines.append("\\multicolumn{7}{c}{\\tablename\\ \\thetable{} -- continued from previous page} \\\\")
        latex_lines.append("\\hline")
        latex_lines.append("\\textbf{Location} & \\textbf{Lat} & \\textbf{Lon} & \\textbf{Flood} & \\textbf{F.Class} & \\textbf{Landslide} & \\textbf{L.Class} \\\\")
        latex_lines.append("\\hline")
        ...
        # Add data rows
        for r in results:
            name = escape_latex(r.name or "Unnamed")
            # Truncate long names
            if len(name) > 20:
                name = name[:17] + "..."
            
            flood_class_short = r.flood_class[:1] if r.flood_class else "-"
            landslide_class_short = r.landslide_class[:1] if r.landslide_class else "-"
            
            latex_lines.append(
                f"{name} & {r.latitude:.4f} & {r.longitude:.4f} & "
                f"{r.flood_susceptibility:.1f} & {flood_class_short} & "
                f"{r.landslide_susceptibility:.1f} & {landslide_class_short} \\\\"
            )
        ...
        return Response(
            ...
            media_type="text/plain",
            headers={
                "Content-Disposition": f"attachment; filename=full_results_{request.session_id[:8]}.tex"
            }
        )
...
# Utility function used in CSV upload (import from study_area if needed)
def is_in_study_area(lat: float, lon: float) -> bool:
    """Check if coordinates are within the study area (always True for global research platform)."""
    # For the research platform, all locations are considered valid
    # The dynamically configured study area is used for analysis context
    return True
