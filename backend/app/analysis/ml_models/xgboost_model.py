...
class LandslideXGBoost:
    ...
    def __init__(self,
        ...
        self.cv_models: List[Any] = []  # For uncertainty quantification
    ...
    def train(self,
              ...
              test_size: float = 0.3,
              cv_folds: int = 5,
              coordinates: Optional[np.ndarray] = None) -> XGBoostMetrics:
        """
        Train the XGBoost model with optional spatial cross-validation.
        ...
        Args:
            ...
            coordinates: Optional (n_samples, 2) array of [x, y] coordinates
        ...
        # Initialize CV models storage for uncertainty
        self.cv_models = []
        ...
        if coordinates is not None:
            ...
            logger.info(f"Using spatial train/test split: {len(train_idx)} train, {len(test_idx)} test")
        else:
        ...
        # Spatial Cross-Validation for CV scores and uncertainty models
        if coordinates is not None:
            from app.analysis.validation import SpatialSplitter
            cv_splitter = SpatialSplitter(coordinates, n_splits=cv_folds)
            cv_scores = []
            for t_idx, v_idx in cv_splitter.split_checkerboard():
                # Clone model with same/best parameters
                if best_params:
                    clf = xgb.XGBClassifier(**{**self.params, **best_params})
                else:
                    clf = xgb.XGBClassifier(**self.params)
                
                clf.fit(X[t_idx], y[t_idx], verbose=False)
                
                # Store for uncertainty quantification
                self.cv_models.append(clf)
                
                score = roc_auc_score(y[v_idx], clf.predict_proba(X[v_idx])[:, 1])
                cv_scores.append(score)
            cv_scores = np.array(cv_scores)
            logger.info(f"Spatial CV complete. Mean AUC: {np.mean(cv_scores):.4f}")
        else:
    ...
    def predict_with_uncertainty(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Predict susceptibility with uncertainty estimates using CV model ensemble.
        
        Uses the ensemble of spatially cross-validated models to provide
        both mean predictions and standard deviation (epistemic uncertainty).
        ...
        Raises:
            ValueError: If no CV models available (train with coordinates first)
        """
        if not self.cv_models:
            raise ValueError(
                "No CV models available for uncertainty estimation. "
                "Run training with coordinates parameter to enable spatial CV."
            )
