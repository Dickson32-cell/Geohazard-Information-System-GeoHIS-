...
class InformationValueAnalyzer:
    """
    Information Value (Weight of Evidence) analyzer for susceptibility mapping.
    
    Uses Laplace smoothing (add-one smoothing) to handle zero hazard cases,
    which provides a theoretically grounded approach rather than arbitrary floor values.
    
    Reference: Manning, C.D., Raghavan, P., & Schütze, H. (2008). Introduction to 
    Information Retrieval. Cambridge University Press. (Chapter 11: Probabilistic IR)
    """
    
    # Laplace smoothing constant (add-one smoothing)
    LAPLACE_ALPHA = 1.0
    ...
    def __init__(self, total_study_area: float, total_hazard_area: float, hazard_type: str = "landslide"):
        ...
        self.prior_probability = total_hazard_area / total_study_area if total_study_area > 0 else 0.0
        self.factors: Dict[str, List[Dict[str, Any]]] = {}
        self.results: Dict[str, FactorIVResult] = {}
    
    def add_factor(self, factor_name: str, class_data: List[Dict[str, Any]]) -> None:
    ...
    def calculate_iv_for_factor(self, factor_name: str) -> FactorIVResult:
        ...
        for cd in class_data:
            ...
            # Use Laplace smoothing for IV calculation to handle zero hazard cases
            # This provides a theoretically grounded approach rather than arbitrary floor values
            # IV = ln((hazard_area + alpha) / (class_area + alpha)) - ln(prior_probability)
            alpha = self.LAPLACE_ALPHA
            
            if class_area > 0 and self.total_study_area > 0:
                # Smoothed density for this class
                smoothed_hazard = hazard_area + alpha
                smoothed_area = class_area + alpha
                smoothed_dens_class = smoothed_hazard / smoothed_area
                
                # Smoothed prior probability
                smoothed_prior = (self.total_hazard_area + alpha) / (self.total_study_area + alpha)
                
                if smoothed_prior > 0:
                    iv = np.log(smoothed_dens_class / smoothed_prior)
                else:
                    iv = 0.0
            else:
                iv = 0.0
