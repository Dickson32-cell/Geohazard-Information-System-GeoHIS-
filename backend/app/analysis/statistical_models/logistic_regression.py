...
class SusceptibilityLogisticRegression:
    def __init__(self, feature_names: List[str], regularization: str = 'l2', 
        ...
        # 'none' is the string literal for no penalty in older sklearn, None in newer
        # We'll use 'l2' as default if not specified or 'none' string if requested
    ...
    def train(self, X: np.ndarray, y: np.ndarray, test_size: float = 0.3, 
              cv_folds: int = 5, coordinates: Optional[np.ndarray] = None) -> Dict[str, Any]:
        ...
        self.cv_models: List[Tuple[Any, Any]] = []
        
        # Use spatial split for train/test if coords provided, else random
        if coordinates is not None:
            from app.analysis.validation import SpatialSplitter
            splitter = SpatialSplitter(coordinates, n_splits=int(1/test_size))
            # Just take the first split for train/test holdout
        ...
        # Cross-validation & Uncertainty Model Collection
        if coordinates is not None:
             # Spatial CV
            from app.analysis.validation import SpatialSplitter
            ...
            for t_idx, v_idx in cv_splitter.split_checkerboard():
                ...
                clf = LogisticRegression(penalty=self.model.penalty, C=self.model.C, 
                                        solver=self.model.solver, max_iter=1000)
                clf.fit(X_t, y[t_idx])
                
                # Store model and scaler for uncertainty prediction
                self.cv_models.append((clf, scaler))
                
                if hasattr(clf, "predict_proba"):
                    score = roc_auc_score(y[v_idx], clf.predict_proba(X_v)[:, 1])
                else:
                    score = clf.score(X_v, y[v_idx])
                cv_scores.append(score)
            cv_scores = np.array(cv_scores)
        else:
            cv_scores = cross_val_score(self.model, X_full_scaled, y, cv=cv_folds, scoring='roc_auc')

        validation = self._calculate_validation_metrics(y_test, y_pred, y_pred_proba, cv_scores)
        ...
        return {
            ...
            'uncertainty_capable': len(self.cv_models) > 0,
    ...
    def _calculate_coefficients(self, X: np.ndarray, y: np.ndarray) -> List[Dict]:
        ...
        for _ in range(200):
            ...
            try:
                ...
                # Skip bootstrap samples that fail to converge
    ...
    def _calculate_validation_metrics(self, y_test, y_pred, y_pred_proba, cv_scores):
        # cast to float to satisfy type checker and prevent numpy scalar issues
        accuracy = float(accuracy_score(y_test, y_pred)) # type: ignore
        precision = float(precision_score(y_test, y_pred, zero_division=0)) # type: ignore
        recall = float(recall_score(y_test, y_pred, zero_division=0)) # type: ignore
        f1 = float(f1_score(y_test, y_pred, zero_division=0)) # type: ignore
        auc = float(roc_auc_score(y_test, y_pred_proba)) # type: ignore
        ...
        return {
            ...
            'cross_val_mean': round(float(np.mean(cv_scores)), 4) if len(cv_scores) > 0 else 0.0,
            'cross_val_std': round(float(np.std(cv_scores)), 4) if len(cv_scores) > 0 else 0.0
        }
    
    def _calculate_validation(self, X_full, y_full, X_test, y_test, y_pred, y_pred_proba, cv_folds):
        # Deprecated in favor of _calculate_validation_metrics but kept for compatibility if needed
        # We redirect to the new method
        return self._calculate_validation_metrics(y_test, y_pred, y_pred_proba, [])
