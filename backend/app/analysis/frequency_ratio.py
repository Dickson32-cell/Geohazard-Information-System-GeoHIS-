...
class FrequencyRatioAnalyzer:
    ...
    def calculate_fr(self, factor_name: str) -> FrequencyRatioResult:
        ...
        max_fr = max(frequency_ratios) if frequency_ratios else 1.0
        normalized_fr = [float(fr / max_fr) if max_fr > 0 else 0.0 for fr in frequency_ratios]
        ...
        for factor_name, class_name in pixel_classes.items():
            if factor_name in self.results:
                ...
                try:
                    ...
                    missing_classes.append(f"{factor_name}:{class_name}")
        
        if missing_classes:
            logger.warning(f"Classes not found in FR results: {missing_classes}. "
                          "These were excluded from LSI calculation.")
    ...
    def calculate_chi_square(self, factor_name: str) -> Dict[str, float]:
        """
        Calculate chi-square statistic to test significance of FR values.
        
        Tests whether the observed distribution of landslides across classes
        differs significantly from what would be expected by chance.
        
        H0: Landslide occurrence is independent of factor class
        H1: Landslide occurrence is related to factor class
        ...
        if n_classes < 2:
            return {
                ...
                'significant': False,
                'interpretation': 'Insufficient classes for chi-square test'
            }
        ...
        total_landslide = self.total_landslide_area
        total_study = self.total_study_area
        overall_rate = total_landslide / total_study if total_study > 0 else 0
        
        for cls in classes:
            observed.append(cls.landslide_area)
            # Expected landslide area if rate were uniform
            expected.append(cls.class_area * overall_rate)
        ...
        # P-value (using scipy if available, else approximate)
        try:
            from scipy import stats
            p_value = 1 - stats.chi2.cdf(chi_square, df)
        except ImportError:
            # Approximate p-value using normal approximation for large df
            z = (chi_square - df) / np.sqrt(2 * df)
            p_value = 1 - 0.5 * (1 + np.tanh(z * 0.7978845608))  # Approx normal CDF
        ...
        # Interpretation
        if p_value < 0.001:
            interpretation = f"Highly significant (p < 0.001): Strong evidence that {factor_name} influences landslide occurrence"
        elif p_value < 0.01:
            interpretation = f"Very significant (p < 0.01): Strong evidence that {factor_name} influences landslide occurrence"
        elif p_value < 0.05:
            interpretation = f"Significant (p < 0.05): Evidence that {factor_name} influences landslide occurrence"
        else:
            interpretation = f"Not significant (p = {p_value:.4f}): Insufficient evidence that {factor_name} influences landslide occurrence"
    ...
    def get_significance_report(self) -> Dict[str, Dict]:
        """
        Generate chi-square significance report for all factors.
        
        Returns:
            Dictionary with chi-square results for each factor
        """
        if not self.results:
            self.calculate_all_factors()
        ...
        report = {}
        for factor_name in self.factors:
            report[factor_name] = self.calculate_chi_square(factor_name)
