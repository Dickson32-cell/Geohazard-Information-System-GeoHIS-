...
import EarthquakeAnalysis from '../components/EarthquakeAnalysis';
...
const AnalyzePage = () => {
    ...
    return (
        <Container fluid className="py-4">
            <Row>
                <Col md={4}>
                    <h2 className="mb-4">Geohazard Susceptibility Analysis</h2>
                    ...
                    <DataManagement onDataChange={() => console.log('Data changed')} />
                    
                    <AnalysisAssistant onAnalysisConfigured={(config) => {
                        console.log('Analysis configured:', config);
                        // Could update study area bounds or trigger analysis
                    }} />
                    
                    <AdvancedAnalysis />
                    
                    <EarthquakeAnalysis onAnalysisComplete={(results) => {
                        // Handle earthquake results - could display on map or show summary
                        console.log('Earthquake analysis results:', results);
                    }} />
                </Col>
                ...
                <Col md={8}>
                    <Card className="h-100">
                        <Card.Header className="d-flex justify-content-between align-items-center">
                            <h5 className="mb-0">Susceptibility Analysis Map</h5>
                            {studyAreaBounds && (
                                <small className="text-muted">
                                    Study Area: {studyAreaBounds.minLat.toFixed(4)}Â° to {studyAreaBounds.maxLat.toFixed(4)}Â° N,
                                    {studyAreaBounds.minLon.toFixed(4)}Â° to {studyAreaBounds.maxLon.toFixed(4)}Â° E
                                </small>
                            )}
                        </Card.Header>
                    ...
                    <Card className="mt-3">
                        <Card.Body>
                            ...
                            <div className="d-flex flex-wrap gap-3">
                                <span><span className="badge" style={{ backgroundColor: '#dc3545' }}>â—</span> Critical</span>
                                <span><span className="badge" style={{ backgroundColor: '#fd7e14' }}>â—</span> High</span>
                                <span><span className="badge" style={{ backgroundColor: '#ffc107' }}>â—</span> Moderate</span>
                                <span><span className="badge" style={{ backgroundColor: '#28a745' }}>â—</span> Low</span>
                                <span><span className="badge" style={{ backgroundColor: '#6c757d' }}>â—</span> Very Low</span>
                            </div>
                        </Card.Body>
                    </Card>
                </Col>
